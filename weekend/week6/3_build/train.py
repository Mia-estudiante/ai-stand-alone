...
from networks.LeNet_network import LeNet_Linear, LeNet_MultiConv, LeNet_Merge, LeNet_Merge2
...
def main():
    ...
    #Step2. 모델 인스턴스 생성
    # model = LeNet_Linear(args.img_size, args.num_classes).to(args.device)
    # model = LeNet_MultiConv(args.img_size, args.num_classes).to(args.device)
    # model = LeNet_Merge(args.img_size, args.num_classes).to(args.device)
    model = LeNet_Merge2(args.img_size, args.num_classes).to(args.device)
    ...
    for epoch in range(args.epochs):
        for idx, (images, targets) in enumerate(train_loader):
            ...
            if idx%100==0:
                ...
                print(f"{epoch} epoch, {idx+1} iteration의 duration: ", sum(durations)/len(durations))
                durations = []

...
def imdb_collate_fn(batch):
    ...
    for sentence in batch_sentences:
        ...
        sentences.append(sentence) #tensor의 형태로 만들어야 함
        lengths.append(len(sentence))
    batch_sentences = pad_sequence(sentences, batch_first=True)  #padding_value: 패딩값 설정
    batch_labels = torch.Tensor(batch_labels).to(torch.int64)    #정수로 형변환
